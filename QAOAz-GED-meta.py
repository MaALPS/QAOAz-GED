...
def optimize():
    ...
    with open("output2.txt", "a") as file:  # 打开文件以追加模式
    ...
    return [x, y, bb0, avg_opt_gap, acc, optimal_params]
...
best_solutions = []
run_time = []
file_path = "Parameter/data/task.pkl"
...
for _ in range(len(data_list)):
    ...
    for run in range(1):
        ...
        x, y, best_solution, avg_opt_gap, acc, optimal_params = optimize()
        best_solutions.append(best_solution)
        avg_opt_gaps.append(avg_opt_gap)
        run_time.append(time.time()-start_time)
        accs+=acc

    task_data.append((Q,optimal_params))
...
print(f"追加后，task.pkl 共有 {len(task_data)} 组任务数据")


with open("output_qaoaz2.txt", "a") as file:  # 打开文件以追加模式
    file.write(f"\ndata:{data_list}:\ndepth:{depth}\n")
    file.write(f"avg_opt_gaps:{avg_opt_gaps}\naccs:{float(accs/len(data_list))}\n"
               f"avg_opt_gap:{float(sum(avg_opt_gaps)/len(data_list))}\navg_time:{float(sum(run_time)/len(run_time))}\n")
